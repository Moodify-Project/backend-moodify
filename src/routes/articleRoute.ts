...
const bookmarkTheArticle = new BookmarkTheArticle(
  userBookmarkArticleRepository,
  articleRepository
);
const fetchAllArticleService = new FetchAllArticleService(
  baseRepository,
  articleRepository,
  userBookmarkArticleRepository
);
const articleBookmarkedByUserService = new ArticleBookmarkedByUserService(
  userBookmarkArticleRepository
);
const fetchBookmarkedArticle = new FetchBookmarkedArticle(
  userBookmarkArticleRepository,
  articleRepository
);
const deleteArticleBookmarked = new DeleteArticleBookmarked(
  articleRepository,
  userBookmarkArticleRepository
);
...
const articleHandler = new ArticleHandler(
  bookmarkTheArticle,
  fetchAllArticleService,
  articleBookmarkedByUserService,
  fetchBookmarkedArticle,
  deleteArticleBookmarked
);
...
articleRouter.get(
  "/:id",
  authMiddleware,
  articleHandler.getSpecificArticleContent
);
articleRouter.post(
  "/bookmark",
  authMiddleware,
  articleHandler.addArticleToBookmark
);
articleRouter.delete(
  "/:id/bookmark",
  authMiddleware,
  articleHandler.deleteBookmark
);
articleRouter.get(
  "/bookmarks/me",
  authMiddleware,
  articleHandler.showAllBookmarkedArticle
);
