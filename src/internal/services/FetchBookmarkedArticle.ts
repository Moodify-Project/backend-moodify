...
export class FetchBookmarkedArticle {
  ...
  constructor(
    userBookmarkArticleRepository: UserBookmarkArticleRepository,
  ...
  findAll = async (email: string): Promise<Article[]> => {
    const idCollections =
      await this.userBookmarkArticleRepository.findBookmarkedArticleByUser(
        email
      );
    ...
    return await this.articleRepository.findAllArticleBookmarkedByUser(
      tempCollection
    );
  };
}
